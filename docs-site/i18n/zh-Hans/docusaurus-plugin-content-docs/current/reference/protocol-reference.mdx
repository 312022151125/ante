...
title: "通信协议参考手册"
sidebar_label: "通信协议"
description: "Ante 客户端与守护进程之间的消息协议、报文传输格式与类型定义的完整参考手册"
---

Ante 在客户端（Client）与守护进程（Daemon）之间采用强类型的消息传递协议。同进程客户端使用有界操作通道和无界事件通道；外部客户端则通过 JSON Lines 或 WebSocket 帧交换相同消息。
...
根据会话 ID 恢复此前持久化保存的会话。守护进程会还原已保存的对话以及固定的模型、提供商与思考深度；快照未固定的配置从主机当前默认值解析。随后重新发现扩展项并回放完整的持久化对话，以便客户端重建视图。

权限模式始终从主机的当前设置解析。旧版快照仍可加载，但其中保存的权限模式会被忽略，且下次保存时不再写入。
...
成功时守护进程会针对当前会话（若有）触发 `SessionEnd`，随后为恢复的会话触发 `SessionStart` 和 `ExtensionRefreshed`，随后按顺序回放完整持久化事件日志，跳过会话生命周期事件和格式错误的行，不再限制为 200 条。失败时触发 `Error` 事件。
...
响应并解决挂起的提问暂停（`TurnPauseReason::Question`）。由客户端在用户回答、跳过或发起讨论 `AskUser` 提出的结构化问题后发送。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `turn_id` | `Id` | 处于暂停状态的轮次 ID |
| `tool_use_id` | `string` | 对应提问的工具调用 ID |
| `reply` | `QuestionReply` | `Answered(QuestionAnswer[])`、`Dismissed` 或 `Discuss { message? }` |

`Answered` 数组按问题顺序逐项对应 `TurnPause.reason.Question.questions`。每个 `QuestionAnswer` 包含：

| 字段 | 类型 | 说明 |
|---|---|---|
| `selected` | `string[]` | 选中选项的完整标签，不是索引；只输入自由文本时为空数组 |
| `note` | `string?` | 补充说明，或 `selected` 为空时的回答正文；未提供文字时可省略 |

`selected` 为空且没有 `note` 表示该问题未回答。跳过整个提示时发送 `"reply": "Dismissed"`；请求讨论时发送 `"reply": { "Discuss": { "message": "先说明这些选项" } }`。`Discuss.message` 可以省略。
...
将运行中的本地推理端点（例如 Ante 托管的 `llama serve` 路由器，或外部运行的 `llama-server`）注册为此守护进程的 `local` 提供商。模型参数为可选 —— 提供时将本地提供商锁定为特定的 `ModelSpec`；否则本地提供商托管该端点提供的任意模型。
...
`ProviderSpec` 有意仅传递客户端无法自行查询的信息。提供商的模型列表不包含在协议消息中；它对每个会话完全相同，并通过 [`ante catalog`](/reference/cli-reference#ante-catalog) 发布。未知字段会被忽略，因此仍会发送 `preferred_models` 的旧版守护进程载荷能够正常兼容解码。
...
| `Question` | `tool_use_id: string`, `questions: QuestionSpec[]` | 正在等待用户回答 `AskUser` 提出的结构化问题 |

**QuestionSpec 字段：**

| 字段 | 类型 | 说明 |
|---|---|---|
| `header` | `string` | 问题的短标签 |
| `question` | `string` | 完整问题正文 |
| `multi_select` | `bool` | 是否允许多选；省略时为 `false`。注意协议字段是 `multi_select`，工具输入字段为 `multiSelect` |
| `options` | `QuestionOption[]` | 提供的选项；“其他”自由文本入口由客户端添加 |

每个 `QuestionOption` 包含 `label` 和 `description` 字符串，以及可选的 `preview` 字符串（代码、示意或配置片段）。提问暂停期间，同一批次中的其他工具仍可能运行；客户端应持续读取事件，并在收到 `TurnResume` 或 `TurnEnd` 时清除未回答的提示。
