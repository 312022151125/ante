...
title: "权限管控系统"
sidebar_label: "权限控制"
description: "Ante 如何通过模式、规则与匹配器控制工具的安全执行"
---
...
恢复会话时使用当前权限设置。历史保存的 Yolo 模式不会覆盖当前的 Strict 或 Auto 模式；旧版快照仍可加载，但其中保存的权限模式会被忽略。

:::warning
无头模式默认使用 `yolo`，但会遵从显式的 `--permission-mode strict` 或 `--permission-mode auto`。由于无头会话无人值守，原本需要审批的调用会被立即拒绝，而不会等待交互提示。参阅[无头模式](/usage/headless)。
:::
...
权限规则将**判定结果（decision）**与**工具匹配器（tool matcher）**配对。持久化规则位于 `~/.ante/settings.json` 的 `permissions.allow`、`permissions.ask` 与 `permissions.deny` 列表中。
...
- **Yes, always allow … (save to settings)** — 将匹配器追加写入 `~/.ante/settings.json` 的 `permissions.allow` 中，以便应用于未来所有会话。
