...
Ante stores user preferences in `~/.ante/settings.json`. A repository can layer its own `.ante/settings.json` on top for everyone working in it — see [Project settings](#project-settings).
...
| `ambient_prompt_suggestion` | Whether the TUI shows a [next-prompt suggestion](/reference/tui-reference#next-prompt-suggestion) as ghost text after a turn. On unless set to `false` |
| `ambient_thinking_phrase` | Whether the TUI predicts a [task-specific spinner phrase](/reference/tui-reference#spinner-thinking-phrase) while you type. On unless set to `false` |
| `tips` | Whether the TUI shows a [tip line](/reference/tui-reference#spinner-tips) under the spinner while the agent works. On unless set to `false` |
...
| `max_concurrent_subagents` | Cap on how many subagents a session runs at once. When the model spawns more `Agent` calls than this in one message, the extra calls wait for a slot (the TUI shows `waiting for a subagent slot`) instead of starting immediately, and the `Agent` tool description tells the model about the cap. Unset, `null`, or `0` means no cap. Set it to `1` for a local model that serves one request at a time, so queued subagents don't sit in the server's queue until [`stream_idle_timeout_secs`](/reference/catalog-reference#provider-fields) fires. Settings-only, applied when starting or resuming a session; see [Concurrency limit](/extend/subagents#concurrency-limit) |
...
| `render_mode` | Chat rendering strategy: `inline` (default, terminal-owned scrollback) or `fullscreen` (alternate screen with Ante-managed scrolling). Changes in `/config` apply live; `--fullscreen` overrides startup for one run without persisting |
...
`ante doctor` prints a `project` row showing the file it resolved (or that none was found) and any keys it dropped. [`ante rage`](/reference/cli-reference#ante-rage) includes the project file, redacted the same way as the user file, so a bug report carries the settings that were actually in effect.
...
The default `inline` mode keeps the chat in your terminal's normal scrollback. Use `fullscreen` when a terminal or console drops inline history: Ante moves the chat to the alternate screen, owns its scrollback, and enables `PgUp` / `PgDn`, mouse-wheel scrolling, and drag-to-copy selection.

```json
{ "render_mode": "fullscreen" }
```

Switch **Render mode** in `/config` to apply and persist the change immediately. To start only the current run in fullscreen without changing settings, use `ante --fullscreen`.
...
| `ANTE_LOCAL_PROVIDER_PORT` | Port assumed for the `local` provider when no live model server registration provides one (default: 8080) |
| `ANTE_HOME` | Override the home config directory (default: `~/.ante`) |
| `ANTE_PROFILE` | Select a named settings profile for the whole process, including subcommands and external applications; equivalent to `--profile <name>` |
...
| `ANTE_MCP_TOOL_TIMEOUT` | MCP `tools/call` deadline in whole seconds. Defaults to `600`; zero or invalid values keep the default (see [MCP tool-call timeout](/extend/mcp#tool-call-timeout)) |
...
| `ANTE_USER` | Optional operator name attached to telemetry as `user.name`; unset by default (see [Identity](#identity)) |
| `ANTE_USER_ID` | Optional operator-chosen identifier attached to telemetry as `user.id`; unset by default |
| `ANTE_ENV` | Group telemetry by deployment environment (default: `local`) |
...
|---|---|---|
| Run id | Random per process, never persisted | Gives each concurrently running Ante its own metric series, which cumulative counters require. Exported as `service.instance.id` on metrics only. |
| Installation id | A random memorable label such as `clever-otter-x7f2`, stored in `~/.ante/installation-id` | Distinguishes "one machine reported 500 errors" from "500 machines did". Minted at the first startup that has telemetry configured, and word-shaped so you can quote it in a bug report — `ante rage` includes it. The words encode nothing about you, your host, or any account. You can edit this file to change its content to a common name like `anon` or anything you like. |
| `user.name` | `ANTE_USER`, explicit only | Names whoever operates a deployment. CI and eval runners set it; a laptop leaves it unset. |
| `user.id` | `ANTE_USER_ID`, explicit only | An operator-chosen identifier when a name isn't the right shape. |

You can edit `~/.ante/installation-id` to change the identifier to a common label like `anon` or any name you prefer, or delete `~/.ante/installation-id` to reset to a fresh identity (it is recreated on the next run that has telemetry configured). A home directory Ante cannot read or write simply reports without one. Deployments can also group data with `ANTE_ENV` (default: `local`).
...
Oversized WebFetch responses spill into the [OS cache directory](/reference/storage-reference#locations), where entries older than 48 hours are collected on a later startup. Disposable staging files and intermediate downloads use the OS temp directory, outside that cache sweep. Changing `ANTE_HOME` normally moves neither location. Background Bash handles live under `~/.ante/run/jobs/<proc-id>/` and are retained while their process is live, then for 24 hours after it settles. Application logs live under `~/.ante/logs/<YYYY-MM-DD>/`, partitioned by UTC date.
...
3. The project's `.ante/settings.json`, for the keys it is [allowed to set](#project-settings)
