...
Resuming a session uses the current permission settings. A saved Yolo mode cannot override your current Strict or Auto mode; legacy snapshots still load, but their saved permission mode is ignored.

:::warning
Headless runs default to `yolo`. An explicit `--permission-mode strict` or `--permission-mode auto` is honored; because headless sessions are unattended, calls that would require approval are denied immediately instead of waiting for a prompt. See [Headless mode](/usage/headless).
:::
