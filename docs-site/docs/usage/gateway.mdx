...
`ante gateway` connects to a running Ante host (`ante serve --sock` or via `--connect`) using the Ante SDK rather than managing an in-process runtime. It reads `~/.ante/channels.json`, connects to every platform you've enabled, and starts listening for messages.

### Standalone gateway from source

The public repository also includes `ante-gateway`, a standalone SDK client with the same channel configuration and conversation behavior. From the repository root:
...
Start `ante serve --sock` first, as above, or use `ante-gateway --connect stdio` to spawn an Ante host for each conversation. For stdio, the executable is resolved from a non-empty `ANTE` environment variable, then `--executable PATH`, then `ante` on `PATH`. The standalone client is built against SDK/protocol version 0.2.1; use a compatible Ante host.

The standalone binary accepts the gateway flags listed below, plus `--executable` for stdio connections. `ANTE_HOME` overrides the default `~/.ante` directory for `channels.json` and `run/serve.sock`; `ANTE_SERVE_TOKEN` supplies the bearer token for WebSocket hosts. Logs go to stderr, while `--output-format` controls protocol events on stdout.

Installing this package does not replace the gateway bundled with Ante: invoke `ante-gateway` directly to run this source build.
...
**Using the bot in Discord:** `@mention` it in a server channel, or just DM it. The bot replies as a reference to your message — reply to the bot's message to continue the conversation (subsequent replies in the thread route back to the original conversation without requiring explicit bot re-mentions).
...
| `no` (or `n`) | Deny this tool call |
| `always` | Approve and allow this tool for the rest of the session |

See [Approvals](/usage/approvals) for more on tool approval flows.

## CLI reference

```bash
ante gateway [--config PATH] [--connect ENDPOINT] [--model MODEL] [--provider PROVIDER] [--output-format FORMAT]
```

| Flag | Default | Description |
| --- | --- | --- |
| `--config` | `~/.ante/channels.json` | Path to the channels config file |
| `--connect` | `unix:~/.ante/run/serve.sock` | Ante host endpoint: `unix:<path>`, `ws://<addr>`, `wss://<addr>`, or `stdio`. Start a host with `ante serve --sock` |
| `--model` | — | Model to request (otherwise uses host settings) |
| `--provider` | — | Provider to request (otherwise resolved by host) |
| `--output-format` | `minimal` | Gateway stdout format: `minimal`, `human`, or `json` |

:::tip
By default, `ante gateway` connects to a local host socket at `unix:~/.ante/run/serve.sock`. To connect to a different host endpoint, pass `--connect` (e.g. `ante gateway --connect unix:/tmp/ante.sock`).
:::
