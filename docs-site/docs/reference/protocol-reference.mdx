...
Ante uses a typed message-passing protocol between the client and daemon. In-process clients use a bounded operation channel and an unbounded event channel; external clients exchange the same messages over JSON Lines or WebSocket frames.
...
Resume a previously persisted session by its ID. The daemon restores the saved dialog and pinned model, provider, and effort, resolves anything the snapshot did not pin from the host's current defaults, re-discovers extensions, and replays the complete persisted conversation so the client can rebuild its view.

Permission mode always resolves from the host's current settings. Older snapshots still load, but their saved permission mode is ignored and omitted on the next save.
...
On success the daemon emits `SessionEnd` for the current session (if any), then `SessionStart` and `ExtensionRefreshed` for the resumed session, followed by the complete persisted event log in order, excluding session lifecycle events and malformed lines. There is no 200-event cap. On failure it emits an `Error` event.
...
Resolve a pending question pause (`TurnPauseReason::Question`). Sent after the user answers, skips, or requests a discussion on structured questions asked by `AskUser`.
...
```json
{
  "op": {
    "QuestionResponse": {
      ...
      "reply": {
        "Answered": [
          { "selected": ["JWT (Recommended)"], "note": null }
        ]
      }
    }
  },
...
| Field | Type | Description |
|---|---|---|
| `turn_id` | `Id` | The paused turn's identifier |
| `tool_use_id` | `string` | Tool call identifier matching the pause |
| `reply` | `QuestionReply` | `Answered(QuestionAnswer[])`, `Dismissed`, or `Discuss { message? }` |

The `Answered` array corresponds to `TurnPause.reason.Question.questions` in question order. Each `QuestionAnswer` contains:

| Field | Type | Description |
|---|---|---|
| `selected` | `string[]` | Exact labels of selected options, not indices; empty for a free-text-only answer |
| `note` | `string?` | Additional text, or the answer itself when `selected` is empty; omit when no text was entered |

An empty selection with no note leaves that question unanswered. To skip the whole prompt, send `"reply": "Dismissed"`. To discuss it, send `"reply": { "Discuss": { "message": "Explain the options first" } }`; `Discuss.message` is optional.
...
`ProviderSpec` deliberately carries only what a client cannot look up for itself. The provider's model list is not on the wire; it is identical for every session and is published by [`ante catalog`](/reference/cli-reference#ante-catalog). Unknown fields are ignored, so payloads from older daemons that still send `preferred_models` decode fine.
...
| `Question` | `tool_use_id: string`, `questions: QuestionSpec[]` | Waiting for user response to structured questions from `AskUser` |
...
| `multi_select` | `bool` | Whether multiple options may be selected; defaults to `false`. The wire field is `multi_select`, while the tool input uses `multiSelect` |
| `options` | `QuestionOption[]` | Offered choices; the client adds its own free-text "Other" input |

Each `QuestionOption` has `label` and `description` strings, plus an optional `preview` string for a code snippet, mockup, or config excerpt. Other tools in the batch may keep running during a question pause. Keep consuming events and clear any pending question on `TurnResume` or `TurnEnd`.
...
| Event channel | Daemon → Client | Unbounded; Ante warns when the pending backlog reaches 4096 and reports when it drains |
