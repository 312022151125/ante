...
- **Transport** — A bounded operation channel and unbounded event channel (Tokio) for in-process clients (TUI, headless), JSONL over stdin/stdout for external clients (`ante serve --stdio`), a Unix domain socket for local clients that dial a shared host (`ante serve --sock`), or WebSocket for networked clients (`ante serve --ws`). Message IDs enable tracing across the boundary.
...
Ante is provider-agnostic. Each provider implements a common interface for sending prompts and receiving responses. Transient mid-stream failures reconnect within the turn. Streaming calls fail after five minutes of transport silence; any transport read, including keepalives and partial tool arguments, resets that timer. Buffered calls must finish within the timeout. That window is per provider — override it with [`stream_idle_timeout_secs`](/reference/catalog-reference#provider-fields).
...
| OpenAI | Responses API | GPT-6 Astra and GPT-5 models |
| OpenAI Compatible | Chat Completions | Custom models |
| OpenAI Subscription | Responses API | GPT-6 Astra and GPT-5.6 (OAuth via Codex) |
...
- **API keys** — Set via environment variables (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `XAI_API_KEY`, `OPENROUTER_API_KEY`, `OPENAI_COMPATIBLE_API_KEY`, `ZAI_API_KEY`, `DEEPSEEK_API_KEY`, `VERTEX_GEMINI_API_KEY`, `ANTIX_API_KEY`)
...
| `AskUser` | Builtin | No | Ask structured questions; pauses attended sessions for an answer |
| `TodoWrite` | Builtin | No | Manage task lists (opt-in) |
