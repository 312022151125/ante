...
| `--version <VERSION>` | Install an exact release instead of the channel's newest release. Accepts values with or without a leading `v`, such as `v0.preview.99` or `0.preview.99`. Conflicts with channel arguments. |
| `-h`, `--help` | Print command help. |
