...
| `--fullscreen` | Start the TUI chat on the alternate screen with Ante-managed scrollback for this run. Does not persist the `render_mode` setting and is ignored in headless mode. |
...
:::caution
`--ws` requires a nonempty `ANTE_SERVE_TOKEN` and accepts loopback addresses only (`127.0.0.0/8`, `::1`). Clients must send `Authorization: Bearer <token>` in the WebSocket upgrade request; a missing or wrong token returns HTTP 401, and handshakes time out after 10 seconds. A non-loopback address such as `0.0.0.0:8080` is refused before the listener binds. To reach the daemon from elsewhere, use an SSH tunnel or an authenticating reverse proxy with TLS (`wss://`).
:::
...
Since v0.2.2, this command dispatches to the separately installed `ante-gateway` application and passes the remaining arguments through unchanged. The gateway connects to an Ante host via the SDK and uses its channels config for Slack and Discord. See [Gateway mode](/usage/gateway#basic-usage) for installation; a missing executable exits with status `127`.

| Flag | Default | Description |
|---|---|---|
| `--config <PATH>` | `~/.ante/channels.json` | Path to the channels configuration file. |
| `--connect <ENDPOINT>` | `unix:~/.ante/run/serve.sock` | Ante host endpoint: `unix:<path>`, `ws://<addr>`, `wss://<addr>`, or `stdio`. Start a host with `ante serve --sock`. |
...
```bash
ante gateway
ante gateway --connect unix:/tmp/ante.sock
...
| `--version <VERSION>` | Install an exact release instead of the channel's newest release. Accepts values with or without a leading `v`, such as `v0.2.2` or `0.2.2`. Conflicts with channel arguments. |
| `-h`, `--help` | Print command help. |
...
With `--no-compress`, the finished report is a directory at `./ante-rage-<timestamp>` or the exact `--output` path. Both formats refuse to overwrite existing output, and the parent directory must already exist. If compression fails, Ante writes a report directory at the archive path with `.uncompressed` appended.
...
| Flag | Description |
|---|---|
| `--output <PATH>` | Exact output path: an archive normally, or a report directory with `--no-compress`. Defaults to `./ante-rage-<timestamp>.tar.gz` or `./ante-rage-<timestamp>`, respectively. |
| `--no-compress` | Write the finished report as a directory instead of a `.tar.gz`. |
