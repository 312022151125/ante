...
This page is the exhaustive reference. For sign-in and API-key setup, see [Connect a provider](/usage/providers). For configuration examples, jump to [Custom catalog](#custom-catalog).

## Built-in providers

The presets below match **v0.2.2**: 15 hosted-provider presets, plus `local` when an offline server is running.
...
The `key` column is what you pass to `--provider`. Providers authenticating via OAuth (`anthropic-subscription`, `openai-subscription`, `antix`) sign in through `/connect` in the TUI. Antix also supports `ante auth login antix` from the CLI; API-key providers read an API key from the listed environment variable.
...
The `openai-subscription` provider (ChatGPT/Codex OAuth) offers `gpt-6-astra` (the default), `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`, all with 272K context and image input. GPT-5.5, GPT-5.4, and GPT-5.4 Mini are no longer in the subscription picker.
...
```bash
ante --provider gemini --model gemini-3.8-flash
```
...
| `qwen/qwen3.8-max-0902` | Qwen 3.8 flagship multimodal reasoning model | 1M |
| `qwen/qwen3.7-plus` | Cost-effective Qwen 3.7 model with vision | 1M |
| `qwen/qwen3.8-flash` | Qwen 3.8 Flash multimodal reasoning model | 1M |
...
| `nvidia/nemotron-3.5-lightning` | Fast specialized agent tasks; text only | 262K |
| `nvidia/nemotron-3-ultra-550b-a55b` | Complex agentic reasoning; text only, medium/high effort | 202.8K |

`openrouter-responses` serves `openai/gpt-6-astra`, `openai/gpt-6-astra-pro`, `openai/gpt-5.6-sol`, `openai/gpt-5.6-sol-pro`, `openai/gpt-5.6-terra`, `openai/gpt-5.6-terra-pro`, `openai/gpt-5.6-luna`, and `openai/gpt-5.6-luna-pro` (1.05M context), plus `openai/gpt-5.3-codex` (400K). The `-pro` ids select OpenRouter's pro reasoning route.

`openrouter-anthropic` serves the Claude family over Anthropic Messages: `anthropic/claude-haiku-4.5` (200K context), plus `anthropic/claude-sonnet-5`, `anthropic/claude-opus-5`, and `anthropic/claude-fable-5.1` (1M).
...
```bash
ante --provider openrouter --model deepseek/deepseek-v4.1-flash
...
```bash
ante --provider deepseek --model deepseek-flash
```
...
| `gpt-5.4-mini` | Faster, cheaper GPT-5.4 model | 400K |
| `gpt-5.4-nano` | Smallest GPT-5.4 model | 400K |
| `deepseek-flash` | DeepSeek V4.1 Flash with native vision | 1M |
| `glm-5.3` | Latest GLM-5.3 model for coding and agentic tasks | 1M |
...
`extra_body` can only add fields — keys Ante already generates for that wire style (`model`, `messages`, `stream`, and friends) are dropped at startup with a notice, and the provider still loads. See [Extra request fields](/reference/catalog-reference#extra-request-fields) for the full reserved list.
...
| `support_vision` | Whether the model accepts image input. Built-in models declare this explicitly; custom models default to `true` when unspecified, and an overlay that omits it keeps the preset value. Set `false` for a text-only endpoint. Images are filtered out before sending to non-vision models without altering saved history; reading an image returns its metadata instead of pixels. |
...
Explicit `--provider` and `--model` flags override the `provider` and `model` values in `~/.ante/settings.json`. When neither source specifies a choice, Ante uses the auto-detection rules below.
...
Effort has no env override — set it per run with the [`--effort` CLI flag](/reference/cli-reference#ante) or per model via the `effort` field.
...
- [Connect a provider](/usage/providers) — sign in with OAuth or an API key
