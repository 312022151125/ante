...
| `support_vision` | Whether the model accepts image input. When omitted, Ante infers it from the model id; images are filtered out before sending to models that do not support vision, preventing API errors without altering saved history (reading an image against a non-vision model returns its metadata instead of pixels). |
