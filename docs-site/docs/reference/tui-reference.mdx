---
title: TUI Reference
description: "Keyboard shortcuts, terminal controls, and detailed TUI behavior"
---

For task-by-task walkthroughs, start with [Interactive TUI](/usage/tui). This page covers keyboard shortcuts and detailed interface behavior.
...
The TUI is built with [ratatui](https://ratatui.rs/) and provides a rich chat interface directly in your terminal. It renders inline (up to 24 lines) by default and uses debounced rendering at approximately 100fps for smooth output. If your terminal drops inline scrollback, run `ante --fullscreen` or switch **Render mode** in `/config`; fullscreen mode uses the alternate screen with Ante-managed scrolling and can be toggled live.
...
When the agent proposes file edits, Ante switches to a fullscreen diff view on an alternate screen. You can review the exact changes before approving. Diff views use numbered gutters, full-width added/removed bands, and `+N -M` change counts in their headers. Completed edit results in the conversation view use a 32-row inline budget that preserves complete diff hunks in file order, with remaining changes summarized and accessible in full via `Ctrl+O`. Runs of more than three unchanged context lines keep only their first and last lines, without an “unchanged lines” placeholder; line numbers still reflect their original positions.
...
- Replays the complete persisted event log to rebuild the conversation view, with no 200-event cap
...
Opening a saved conversation from the terminal restores its history so you can continue where you left off:

<a href="/assets/cookbook/resume-demo-2.gif" target="_blank" rel="noreferrer">
  <img src="/assets/cookbook/resume-demo-2.gif" alt="Opening the saved-session picker from the terminal and restoring a conversation" className="cookbook-demo" loading="lazy" />
</a>
...
| `Ctrl+V` / `Ctrl+Shift+V` | Paste text or an image from the clipboard |
...
The command runs in your own login shell (`$SHELL -lc`, falling back to `/bin/sh`) from the current working directory, with a 120-second timeout. The command appears in the conversation as soon as it is invoked, before it finishes. Its output is fed to the agent as context for the next turn, so you can run a command yourself and then ask the agent to act on what it printed.
...
For an illustrated walkthrough of orchestrating agents, forking conversations, and keeping interactive subagents available, see [Orchestrate agents with /term](/usage/tui#terminal-sessions).
...
While you type a longer prompt, Ante predicts a short, task-specific phrase and uses it as the spinner label for that turn instead of the stable default `Running…` label. Like the next-prompt suggestion, it runs best-effort on the cheapest model off the critical path, and alarming or destructive words are filtered out of the phrase. On by default; disable with `"ambient_thinking_phrase": false` in `~/.ante/settings.json`.
...
Tips come from a small curated list (features, keyboard shortcuts, workflow habits) — no model call is involved. Selection is deterministic: the least recently shown eligible tip wins, each tip has a cooldown measured in app restarts so it doesn't repeat session after session, and a tip shown once stays quiet for the rest of that session. The spinner appears after about 300 ms without new activity; interrupt and token details appear after three seconds of turn execution, and elapsed time appears after ten seconds. Tips appear after five seconds without activity; new activity resets that wait, and tips never enter the transcript.
