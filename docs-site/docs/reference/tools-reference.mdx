...
By default Ante enables `Read`, `Bash`, `Grep`, `Glob`, `TodoWrite`, `AskUser`, `WebFetch`, `Edit`, `Write`, and `Agent`. `WebSearch` is added automatically when the session's provider supports native web search.
